...
const w = { mounted(e2, t2) {
  t2.value && e2.addEventListener("mousedown", (t3) => {
    ...
    let u2 = Math.max(r2.width, r2.height), d2 = t3.clientX - r2.left - u2 / 2, c2 = r2.height / 2 - u2 / 2;
  ...
  function c2(e2) {
    e2.preventDefault(), e2.stopPropagation(), a2.loading || a2.disabled || s2("click", e2);
  }
  function p2(e2) {
    e2.preventDefault(), e2.stopPropagation(), a2.loading || a2.disabled || s2("focus", e2);
  }
  function f2(e2) {
    e2.preventDefault(), e2.stopPropagation(), a2.loading || a2.disabled || s2("focus", e2);
  }
  return () => o(l("button", { class: r2.value, style: u2.value, onClick: c2, onFocus: p2, onBlur: f2 }, [l("span", { class: "m-button-content" }, [l("div", { class: "m-click__res", "data-disabled": a2.loading || a2.disabled ? "on" : "off" }, null), a2.loading && l("span", { class: "m-button__loading_wrapper", style: { marginRight: d2.value ? "8px" : 0 } }, [l("svg", { viewBox: "0 0 1024 1024", class: "m-button-icon__loading" }, [l("path", { d: "M980.752 313.697c-25.789-60.972-62.702-115.725-109.713-162.736-47.012-47.011-101.764-83.924-162.736-109.713C645.161 14.542 578.106 1 509 1c-2.242 0-4.48 0.015-6.715 0.043-16.567 0.211-29.826 13.812-29.615 30.38 0.209 16.438 13.599 29.618 29.99 29.618l0.39-0.002c1.98-0.026 3.963-0.039 5.95-0.039 61.033 0 120.224 11.947 175.93 35.508 53.82 22.764 102.162 55.359 143.683 96.879s74.115 89.862 96.88 143.683C949.054 392.776 961 451.967 961 513c0 16.568 13.432 30 30 30s30-13.432 30-30c0-69.106-13.541-136.162-40.248-199.303z", fill: "currentColor" }, null)])]), d2.value ? i2.default && i2.default() : null])]), [[n("down-res"), a2.clickRes]]);
...
} }), L = a({ name: "MHeader", props: { height: { type: String, default: "64px" } }, setup: (e2, { slots: t2 }) => () => l("header", { class: "m-header", style: { "--height": e2.height } }, [t2.default && t2.default()]) }), E = a({ name: "MSide", props: { width: { type: String, default: "200px" } }, setup: (e2, { slots: t2 }) => () => l("nav", { class: "m-side", style: { "--size": e2.width } }, [t2.default && t2.default()]) }), M = a({ name: "MFooter", props: { height: { type: String, default: "64px" } }, setup: (e2, { slots: t2 }) => () => l("nav", { class: "m-footer", style: { "--size": e2.height } }, [t2.default && t2.default()]) }), P = a({ name: "MMain", setup: (e2, { slots: t2 }) => () => l("main", { class: "m-main" }, [t2.default && t2.default()]) }), z = a({ name: "MAnchor", props: { getTarget: { type: Function, default: () => window } }, setup(t2, { slots: a2 }) {
  e(_);
  return () => l("div", null, [a2.default && a2.default()]);
} }), T = a({ name: "MConfigProvider", props: { locale: { type: String, default: "zh-CN" }, size: { type: String, default: "small", validator: (e2) => ["small", "mini", "large"].includes(e2) } }, setup: (e2, { slots: t2 }) => (i(_, e2), () => l(s, null, [t2.default && t2.default()])) }), R = a({ name: "MBreadcrumb", props: { separator: { type: String, default: "/" }, color: String, activeColor: String }, setup(e2, { slots: a2 }) {
  const o2 = t(() => {
    var _a, _b;
...
} }), $ = a({ name: "MBreadcrumbItem", props: { title: { type: String, default: "" }, disabled: Boolean }, emits: ["click"], setup(e2, { emit: t2, slots: a2 }) {
  function o2(a3) {
...
}), A = (e2, t2, a2, o2 = 0) => new Promise(async (l2, n2) => {
...
}), N = { hideTime: 200, showTime: 250, bgShow: async (e2) => {
  await A(e2, [{ transform: "scaleY(0.88)", offset: 0 }, { transform: "scaleY(1)", offset: 0.33 }, {}], { duration: 250 }).catch(() => {
  });
}, cShow: async (e2) => {
  await A(e2, [{ offset: 0, opacity: 0 }, { offset: 0.33, opacity: 0 }, { offset: 1, opacity: 1 }], { duration: 250 }).catch(() => {
  });
}, bgHide: async (e2) => {
  await A(e2, [{ opacity: 1, transform: "scaleY(1)", offset: 0 }, { opacity: 0.66, transform: "scaleY(1)", offset: 0.33 }, { opacity: 0, transform: "scaleY(0.88)", offset: 1 }], { duration: 200 }).catch(() => {
  });
}, async cHide(e2) {
  await A(e2, [{ offset: 0, opacity: 1 }, { offset: 0.33, opacity: 0 }, { offset: 1, opacity: 0 }], { duration: 200 }).catch(() => {
  });
} };
function j(e2, t2, a2) {
...
const q = a({ name: "MDropdown", props: { placement: { type: String, default: "bottomLeft" }, trigger: { type: String, default: "click", validator: (e2) => ["hover", "click"].includes(e2) }, destroyOnHide: { type: Boolean, default: false }, disabled: Boolean, open: { type: Boolean, default: false }, toBody: { type: Boolean, default: true } }, setup(e2, { slots: a2 }) {
  const n2 = r(null), i2 = r(null), g2 = r(null), v2 = r(false), h2 = u({ top: "0", left: "0", waitChange: false, renderToDom: null }), b2 = r(false);
  let w2 = null;
  const _2 = t(() => !e2.destroyOnHide || v2.value);
  let k2 = null;
  function S2() {
    const t2 = H(k2), a3 = H(g2.value), { top: o2, left: l2 } = j(e2.placement, t2, a3);
    h2.top = `${o2}px`, h2.left = `${l2}px`, i2.value.style.minWidth = `${t2.width}px`;
  }
  function x2() {
    _2.value && S2();
  }
  async function C2() {
    v2.value = true, b2.value = true, w2 && (clearTimeout(w2), w2 = null), h2.waitChange = true, await new Promise((e3) => requestAnimationFrame(e3)), S2(), window.addEventListener("scroll", x2, false), N.cShow(g2.value), N.bgShow(i2.value), await F(g2.value).catch(() => {
    }), h2.waitChange = false;
  }
  function B2() {
    if (h2.waitChange || e2.disabled) return;
    !v2.value ? C2() : E2();
  }
  function L2() {
    h2.waitChange || e2.disabled || v2.value || C2();
  }
  function E2() {
    h2.waitChange || (b2.value && (clearTimeout(w2), w2 = null), b2.value = true, w2 = setTimeout(async () => {
      var e3;
      b2.value && (h2.waitChange = true, N.cHide(g2.value), N.bgHide(i2.value), await (e3 = 0, new Promise((t2) => setTimeout(t2, e3))), await F(g2.value).catch(() => {
      }), window.removeEventListener("scroll", x2, false), v2.value = false, h2.waitChange = false);
    }, 60));
  }
  function M2(e3) {
    const t2 = e3.target;
  ...
  return d(() => e2.trigger, () => {
    ...
    T2();
  }), () => l(s, null, [l("div", { class: "m-dropdown", ref: n2, style: { "pointer-events": h2.waitChange ? "none" : "all" } }, [a2.default && a2.default()]), l(m, { to: "#m-inset__body" }, { default: () => [_2.value && o(l("div", { class: "m-dropdown__content m-box__shadow", ref: i2, style: { "--top": h2.top, "--left": h2.left, "pointer-events": h2.waitChange ? "none" : "all" } }, [l("div", { class: "m-dropdown__body", ref: g2 }, [a2.content && a2.content()])]), [[y, v2.value]])] })]);
...
const V = a({ name: "MTransition", props: { onEnter: Function, onLeave: Function }, setup: (e2, t2) => () => {
  ...
  A(e2, [{ opacity: 0 }, { opacity: 1 }], 240), A(a2, [{ transform: "scale(0.33)", offset: 0, opacity: 0.66 }, { transform: "scale(0.98)", offset: 0.66, opacity: 1 }, { transform: "scale(1)", offset: 1, opacity: 1 }], { duration: 240 }), await A(t2, [{ opacity: 0, offset: 0 }, { opacity: 0.05, offset: 0.5 }, { opacity: 1, offset: 1 }], { duration: 240 });
}, I = async ({ mask: e2, wrapper: t2, content: a2 }) => {
  A(e2, [{ opacity: 1 }, { opacity: 0 }], { duration: 200 }), A(a2, [{ opacity: 0.25, offset: 0 }, { opacity: 0.05, offset: 0.33 }, { opacity: 0, offset: 1, transform: "scale(0.33)" }], { duration: 200 }), await A(t2, [{ opacity: 1, offset: 0 }, { opacity: 0.88, offset: 0.33 }, { opacity: 0, offset: 1, transform: "scale(0.33)" }], { duration: 200 }, 20);
  ...
  const a2 = async () => {
    ...
    await A(e3, [{ opacity: 0, offset: 0 }, { opacity: 1, offset: 1 }], { duration: 300 }), t3();
  }, i2 = async (e3, t3) => {
    await A(e3, [{ opacity: 1, offset: 0 }, { opacity: 0, offset: 1 }], { duration: 200 }, 60), t3();
  };
  return () => {
...
} }), Z = [S, x, C, B, L, E, M, P, z, T, R, $, q, D, O, G, Q, U].map((e2) => (e2.install = function(t2) {
